...
function FLYSIM
    ...
    while(ishandle(fig))
        ...
        z = pos(3)-200;
        if (z < 0 || z < GetZ(s1, pos) || z < GetZ(s2,pos) || z < GetZ(s3,pos)) && alarmKlar
            alarm()
        elseif (z > 0 || z > GetZ(s1, pos) || z > GetZ(s2,pos) || z > GetZ(s3,pos)) && ~alarmKlar
            EngineSound();
        end
    ...
    %% Check kwh left & use more fuel per 1000 px risen
    function UpdateFuel
        l = round((pos(3)/1000));
        if (kwt < 0)
            EngineStop();
        else
            kwt = kwt + (0.005 - ((vel*vel/10000000) * (1.05^l)));
        end
    end

    function [fTC]= TestLanding()
        z = pos(3)-10;
        x=abs(pos(1));
        y=abs(pos(2));
        angle=asind(z/sqrt(x^2+y^2+z^2));
        
        
        
        if  (z<100 ||  z < GetZ(s2,pos)) && (vel<200 && angle<15)
            ColorPlane();
             fTC= true;
       
        elseif (z<100 ||  z < GetZ(s2,pos)) && (vel>200 && angle>15)
            TestCrash();
            fTC = true;
        else
            fTC=false;
    ...
    function textureChange
        if mod(b,3) == 1
            ...
            s3.CData = textureSnow;
            sufFlat.CData = textureSnow;
